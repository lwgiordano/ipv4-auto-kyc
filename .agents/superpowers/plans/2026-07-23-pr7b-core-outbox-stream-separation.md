...
- **Delivery-layer only — NO scoring/gate/decision semantic change.** The callback HTTP body stays **byte-identical** to pre-7b: `decision_sequence` is written to the `decisions`/`outbox` **columns only**, never added to `payload_json` or the wire (that is 7b-activation / `021`).
...
remain on 012 — activation (`021`) is downstream and cannot repair this. It NEVER writes.
...
- Produces: docs + governance only. The lineage invariant (head ∈ shipped, `pending[0]` = head+1) was established in Task 1; head is now `020` (redaction uniformity) and `pending[0]=021` (7b-activation).
...
(Leave 7b-activation `021` and every other row unchanged. Also update the prose note near line 12-14 if it says "7b-core is next" — change to "7b-core shipped (`013`-`018`); 7b-activation (`021`) is next".)

- [ ] **Step 3: Re-run the lineage guard after the docs edits**

Run: `.venv/bin/pytest tests/unit/test_migration_lineage.py -v`
Expected: PASS (`head 020 ∈ shipped`; `pending[0]=021=head+1`; disjoint/exhaustive/contiguous all hold).
...
closed here and remain expected until 7b-activation (`021`) adds the platform high-water mark:
...
4. Run `python -m alembic -c alembic.ini upgrade head` (the chain `013`→`014`→`015`→`016`→`017`→`018`→`019`→`020`) — the deployment image runs its exact
...
R4. **With `020` installed there is no schema-downgrade path**: `018`, `019` and `020` refuse unconditionally
...
R5. ROLLBACK OUTCOME A — downgrade REFUSED (any sentinel above): the DB stays on the
    witness-authority schema, so KEEP or redeploy the reviewed **`020`-COMPATIBLE image** digest —
...
def _head_db(pg, name):
    ...
    eng.dispose()
    assert head == "020", f"the documented rollback must start from the REAL head, got {head}"
    return url
...
```bash
# ROADMAP: 7b-core shipped/013-020, 7b-activation still pending/021, no contradictory allocation
...
- [ ] Confirm `rg -n "013" .agents/ROADMAP.md` shows no contradictory live allocation and the 7b-core chain (`013`-`020`) is `shipped`, 7b-activation `021` still `pending`.
