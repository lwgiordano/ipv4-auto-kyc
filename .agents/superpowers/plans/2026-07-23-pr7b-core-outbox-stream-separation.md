...
- **Delivery-layer only — NO scoring/gate/decision semantic change.** The callback HTTP body stays **byte-identical** to pre-7b: `decision_sequence` is written to the `decisions`/`outbox` **columns only**, never added to `payload_json` or the wire (that is 7b-activation / `019`).
...
remain on 012 — activation (`019`) is downstream and cannot repair this. It NEVER writes.
...
- Produces: docs + governance only. The lineage invariant (head ∈ shipped, `pending[0]` = head+1) was established in Task 1; head is now `018` (the transition authority, re-audit `cbb783b`) and `pending[0]=019` (7b-activation).
...
(Leave 7b-activation `019` and every other row unchanged. Also update the prose note near line 12-14 if it says "7b-core is next" — change to "7b-core shipped (`013`-`018`); 7b-activation (`019`) is next".)

- [ ] **Step 3: Re-run the lineage guard after the docs edits**

Run: `.venv/bin/pytest tests/unit/test_migration_lineage.py -v`
Expected: PASS (`head 018 ∈ shipped`; `pending[0]=019=head+1`; disjoint/exhaustive/contiguous all hold).
...
closed here and remain expected until 7b-activation (`019`) adds the platform high-water mark:
...
4. Run `python -m alembic -c alembic.ini upgrade head` (the chain `013`→`014`→`015`→`016`→`017`→`018`) — the deployment image runs its exact
...
R4. **With `018` installed there is no schema-downgrade path**: `018` refuses unconditionally
    ...
    reached `018`: run `python -m alembic -c alembic.ini downgrade 012` (the revision is a
...
R5. ROLLBACK OUTCOME A — downgrade REFUSED (any sentinel above): the DB stays on the
    witness-authority schema, so KEEP or redeploy the reviewed **`018`-COMPATIBLE image** digest —
...
def _head_db(pg, name):
    ...
    eng.dispose()
    assert head == "018", f"the documented rollback must start from the REAL head, got {head}"
    return url
...
```bash
# ROADMAP: 7b-core shipped/013-018, 7b-activation still pending/019, no contradictory allocation
...
- [ ] Confirm `rg -n "013" .agents/ROADMAP.md` shows no contradictory live allocation and the 7b-core chain (`013`-`018`) is `shipped`, 7b-activation `019` still `pending`.
