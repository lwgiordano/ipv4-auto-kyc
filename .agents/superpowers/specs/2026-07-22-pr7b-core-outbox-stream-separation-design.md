...
hardening) and **7b-activation** (migration `021`, `down_revision='020'` — the repair `014`, hardening `015`, admission `016`, authority-boundary `017` and transition-authority `018` revisions sit between them; see §attempt authority — the platform-authoritative cutover — bootstrap,
...
- **Legacy backfill — ordered by `outbox.id`, not `decided_at` (rev-4 F1):** `decided_at` is
  ...
  *lower* sequence, and the local guard would suppress the actually-newer callback (the exact loss this
  ...
  `BLOCKED_NO_AUTHORITATIVE_MAPPING`** (§Rollout step 0; activation `021` is downstream and cannot repair this),
...
pre-activation and are turned into platform-high-water no-ops only in 021 (whose acceptance is
...
(the exact sentinel — one token, no whitespace). **activation (`021`) is downstream and cannot
...
"retain everything until 016 bootstraps" is a temporal band-aid that reopens the hole afterwards; a
...
reviewed `020`-compatible image on schema `018`** — an older publisher lacks the receipt/terminal
...
digest, `013` on `superseded`/digest/amended-history) — that walk is superseded and unreachable once
`018` is on the schema; (5) deploy the pre-7b image **only after** downgrading below `013` on a schema
that never reached `018` (the historical walk above, terminating at `012`). Redeploying the pre-7b
image *before* 013 is applied is also safe. (7b-activation
...
- **Restore durability under retention (rev-10 P1/F4):** restore an **actually retention-old** row
  ...
  intact, and that 016's manifest entry
...
- **No-backup blocked state (rev-6 P2; user-confirmed restore-or-block):** a missing mapping with **no**
  ...
  schema/data **unchanged**; the test proves **no existing `016` CLI/module** is invoked as a substitute
...
- **Residual-risk (F2 — pins the honest boundary):** fake receiver; single publisher; seq 2 sent,
  ...
  **expected pre-activation** (the future 021 test turns the same replay into a high-water no-op).
...
- **F4 — Third residual (§5, §Invariants):** manual-current vs late automatic callback documented
  ...
  no sequence for manual rows; activation's acceptance is strengthened against it (activation spec rev 2).
...
- **P1 — The restore had no durable authority for 016 (§1, §Rollout).** An exact restore reinstates
  the original 7-year-old `delivered_at`, so `retention.py:29-35` re-deleted the row on its next run;
  the plan's `delivered_at=now()` hid this. 016's manifest then had no source for per-callback
  ...
  until 016 bootstraps" (a temporal band-aid that reopens the hole afterwards). It retains no new
