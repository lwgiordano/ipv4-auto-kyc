...
hardening) and **7b-activation** (migration `015`, `down_revision='014'` — the repair revision `014` sits between them; see §attempt authority — the platform-authoritative cutover — bootstrap,
...
- **`outbox.{callback_wire_sha256 TEXT NULL, wire_version TEXT NULL}` — the recorded digest of the
  ...
  construction**, and 015's bootstrap must seed their high-water from `decision_sequence` +
  `local_status` without a digest comparison rather than pretend otherwise.
  This is what lets retention destroy the body while 015 keeps a witness — see the durable-authority
  contract in §Rollout — and it is why 015 never re-encodes a stored payload.
...
- **`outbox_delivery_attempts` — the pre-HTTP attempt authority (rev 13, re-review `45ad8b9` F1;
  shipped in repair revision `014`, NOT in `013`).** An earlier fold amended the committed `013`
  in place, which silently skips the edit on any database already stamped `013`; `013` is now
  restored to its committed shape and frozen by hash, and the attempt authority — plus the
  `witness_generation` marker, the digest⇒delivered CHECK, the latest-decision pointer, and the
  live-status index — lands in `014` (`down_revision='013'`), which creates the table when absent
  and adopts a validated copy when the amended history left one behind.
  ...
  **What an attempt row proves — exactly staged intent, nothing more (re-audit F9).** The commit
  ...
  records durable intent for bytes that never left the machine. So an attempt row means "these
  exact bytes MAY have reached the platform — ask its ledger", never "these bytes were
  transmitted". The state is named `send_intent_witnessed` for that reason; the earlier
  name — `attempt_witnessed`, now retired — and the earlier "handed to the network" prose
  both overclaimed.

  Rows are **insert-only, enforced by a DB trigger (`014`)**: UPDATE always refused; DELETE
  refused unless the parent row carries a terminal digest (then the attempt is redundant and
  retention may reclaim it — for any other row it is the sole evidence of staging). There is
  deliberately **no "winning attempt" link column**. Two attempts for one row carry identical
  bytes (the payload is immutable and the timestamp lives in a header, outside the digest), so
  they are one event to the receiver; a winner flag would be a derived fact free to contradict
  the digest it was derived from — the exact drift class rev 10 rejected. What binds terminal to
  attempt instead is the terminal fence itself: `_record_delivered` stamps a digest only when an
  attempt with the SAME claim token, digest, and encoding exists (re-audit F6), and a digest is
  only writable on a `delivered` row (`ck_outbox_wire_witness_delivered`).

  **Wire witness taxonomy.** Four states, defined once in `src/kyc_tool/outbox/witness.py` and
  selected — never restated — by everything that classifies a row. The classifier needs
  `outbox.witness_generation` (`legacy` | `attempt_v1`, `014`) because absence of an attempt means
  OPPOSITE things across the regime boundary: the pre-authority publisher sent HTTP before any
  durable record, so a `legacy` pending/dead row may well have reached the platform (re-audit F2 —
  it must NEVER classify as never-transmitted), while an `attempt_v1` row with no attempt provably
  staged nothing. The states: `delivery_witnessed` (terminal digest present),
  `send_intent_witnessed` (attempt committed, no terminal digest — only the platform's ledger
  settles it), `legacy_unwitnessed` (any pre-authority row without a digest, in ANY status —
  reconciled on ids and local status alone), `not_accepted` (`attempt_v1` with no attempt — the
  only state in which the tool may assert non-delivery on its own evidence).
...
- **The name is load-bearing: this is NOT a digest of the callback body that was sent (rev 11,
  ...
  one that merely echoes our SQL value proves nothing. 015's wire digest is a separate, versioned
...
- **Triple identity + per-case sequence uniqueness (F1):** four constraints, each load-bearing —
  ...
  **`UNIQUE decisions(case_id, decision_sequence)` named `uq_decisions_case_decision_sequence`** — the
  ...
  case=C, seq=1)` and `(run=B, case=C, seq=1)` satisfy it, and after 015 the second to reach the
...
- **Legacy backfill — ordered by `outbox.id`, not `decided_at` (rev-4 F1):** `decided_at` is
  ...
  *lower* sequence, and the local guard would suppress the actually-newer callback (the exact loss this
  ...
  `BLOCKED_NO_AUTHORITATIVE_MAPPING`** (§Rollout step 0; 015 is downstream and cannot repair this),
...
pre-activation and are turned into platform-high-water no-ops only in 015 (whose acceptance is
...
(the exact sentinel — one token, no whitespace). **activation (`015`) is downstream and cannot
repair this** — no `015` command is a substitute. There is **no** pre-013 reconciliation unit in this
...
storage/ownership decision 015 depends on. Before rev 10 an exact restore was self-defeating: it
...
and the tool then had no way to derive 015's per-callback `(wire digest, local_status)` from the
...
- **What 015 actually consumes is the DIGEST, not the body.** Its manifest entry is
...
- **This is what makes the retention story honest instead of argued.** The callback body carries
  ...
  SHA-256 and a set of internal ordinals — pseudonymous, still joinable to the case, and
  retaining THAT remainder past the window is itself a governed decision: `AUDIT_FINDINGS.md`
  D9 records what survives, why (reconciliation authority), what is destroyed, that the
...
- **It also removes 015's ability to get history wrong.** Because the digest is recorded at send
  time, 015 never re-encodes a stored payload — so 015's `decision_sequence` payload backfill
...
"retain everything until 015 bootstraps" is a temporal band-aid that reopens the hole afterwards; a
...
- **Restore durability under retention (rev-10 P1/F4):** restore an **actually retention-old** row
  ...
  intact, and that 015's manifest entry
...
- **No-backup blocked state (rev-6 P2; user-confirmed restore-or-block):** a missing mapping with **no**
  ...
  schema/data **unchanged**; the test proves **no existing `015` CLI/module** is invoked as a substitute
...
- **Residual-risk (F2 — pins the honest boundary):** fake receiver; single publisher; seq 2 sent,
  ...
  **expected pre-activation** (the future 015 test turns the same replay into a high-water no-op).
...
- **F4 — Third residual (§5, §Invariants):** manual-current vs late automatic callback documented
  ...
  no sequence for manual rows; 015's acceptance is strengthened against it (activation spec rev 2).
...
- **P1 — The restore had no durable authority for 015 (§1, §Rollout).** An exact restore reinstates
  the original 7-year-old `delivered_at`, so `retention.py:29-35` re-deleted the row on its next run;
  the plan's `delivered_at=now()` hid this. 015's manifest then had no source for per-callback
  ...
  until 015 bootstraps" (a temporal band-aid that reopens the hole afterwards). It retains no new
