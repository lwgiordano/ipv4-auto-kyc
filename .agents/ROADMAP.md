...
7b-activation (`016`, the platform-authoritative cutover; `014`/`015` are the shipped
witness-repair and witness-authority revisions — see §C); 7b-core is next.**
...
| PR 7b-activation | 8 | pending | 016 | wire `decision_sequence` emission + `integrity_mismatch`; platform high-water bootstrap (candidate manifest + signed response envelope); `outbox_ordering_activation` phase machine + immutable `BYTEA` artifacts; four CAS CLIs + activation cutover (`down_revision='015'`, forward-only-after-use) |
| PR 6b | 7B | pending | 017 | revalidation / rollout staging |
| PR 7a | 9 | pending | 018 | `jobs.lease_token` |
| PR 8 | 10 | pending | 019 | `adapter_results.{source_sha256,source_size,source_content_type,source_version_id,evidence_ref}` |
| PR 9a/b/c | 12 | — | — | contract + adapter-output validation + real providers |
| PR 10 | 13 | pending | 020 | broker full-list snapshots, `runs.{matched_broker_entity_id,matched_identifier_class,broker_snapshot_revision}` |
...
(user-confirmed 2026-07-23; no pre-013 reconciliation unit; 7b-activation/`016` is downstream). A **step-0 pre-window
...
Migration **016** (`down_revision='015'`): `outbox.failure_class`; the `outbox_ordering_activation`
...
Migration **017** (`down_revision='016'`). Revalidate immutable evidence under the run's pinned
...
Migration **018** (`down_revision='017'`): `jobs.lease_token`. Every transition gated on
...
Migration 019. Structured `ObjectRef{key, sha256, size?, content_type?, version_id?}`
...
Migration 020: broker **full-list immutable snapshots** `(revision, sha256,
