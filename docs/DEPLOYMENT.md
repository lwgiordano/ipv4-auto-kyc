...
- With a migration: revisions downgrade cleanly
  ...
  **Exception — migrations 013-021 (PR 7b-core) are forward-only after any wire
  ...
  sentinels, in execution order — and `018` through `021` refuse UNCONDITIONALLY
  (`MIGRATION_0{18,19,20,21}_DOWNGRADE_REFUSED_FORWARD_ONLY`), because walking below them
  ...
  **021-compatible** image — an older publisher lacks the receipt/terminal
  ...
  `018` through `021` ARE installed, the supported rollback is redeploying the prior
  reviewed `021`-compatible image against schema `021`; the schema does not move.
